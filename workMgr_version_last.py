...
label_title_1 = tk.Label(window, text="添加工作记录",
                         font=(10,),
                         padx=1, pady=1,
                         borderwidth=2,relief='raised').grid(row=0, column=0,ipadx=10,ipady=3)
...
frame3.grid(row=2)
#稿件基础信息
label_book_title = tk.Label(window, text="添加稿件基础信息",font=(10,),padx=1, pady=1,borderwidth=2,relief='raised').grid(row=4,ipadx=10,ipady=3)
...
label_showinfo_text=tk.StringVar()
...
def chooseTypeEvent(event):
    widget = event.widget  # 当前的组件
    value = widget.get()  # 选中的值
    if value=="原稿寄回"or value=="校样寄回":
        entry_pagenum['state']='disable'
    else:entry_pagenum['state']='normal'
    return
...
box_type.bind('<<ComboboxSelected>>',chooseTypeEvent)


def booknameProcess(bookname):
    len_t=len(bookname)
    if(len_t<=10):result=bookname
    else:
        result=bookname[0:5]+"……"+bookname[len_t-4:len_t]
    return result


def chooseNameEvent(event):
    widget = event.widget  # 当前的组件
    value = widget.get()  # 选中的值
    if '原稿' in value:box_type['value'] = ('第一遍加工', '第二遍加工', '原稿寄回')
    elif '校样' in value:box_type['value'] = ('读校样','校样寄回')
    else:box_type['value'] = ('第一遍加工', '第二遍加工', '原稿寄回', '读校样')
    cmd_select_from_book_scores="select * from book_scores where 稿件名称=(?)"
    cur.execute(cmd_select_from_book_scores,(value,))
    result_book_scores = cur.fetchone()
    #bookname=str(result_book_scores[1])
    booknum=str(result_book_scores[2])
    bookname=booknameProcess(str(result_book_scores[1]))
    res_text_01="稿件《"+bookname+"》共"+booknum+"页，"
    cmd_select_from_scores="select 稿件名称,工作类型,sum(加工页数) from scores where 稿件名称=(?) group by 稿件名称,工作类型"
    cur.execute(cmd_select_from_scores, (value,))
    result_scores = cur.fetchall()
    sendedTag=False
    firstEndTag=False
    secondEndTag=False
    firstNum=0
    secondNum=0
    res_text_all=res_text_01
    res_len=len(result_scores)
    for i in range(0,res_len):
        if result_scores[i][1]=='第一遍加工':
            firstNum=result_scores[i][2]
            res_text_02 = "第一遍已加工" + str(firstNum) + "页，"
            res_text_all=res_text_all+res_text_02
        if result_scores[i][1] == '第二遍加工':
            secondNum = result_scores[i][2]
            res_text_03="第二遍已加工"+str(secondNum)+"页，"
            res_text_all=res_text_all+res_text_03
        if result_scores[i][1] == '原稿寄回'or result_scores[i][1] == '校样寄回':sendedTag = True
    if sendedTag:
        res_text_04 = "已寄回。"
    else:
        res_text_04 = "未寄回。"
    res_text_all = res_text_all + res_text_04
    #print(result_scores)
    #print(res_text_all)
    label_showinfo_text.set("*"+res_text_all)
    #del res_text_03,res_text_04,res_text_02,res_text_01,res_text_all
    return

entry_name.bind('<<ComboboxSelected>>',chooseNameEvent)
...
label_showinfo=tk.Label(frame1,textvariable=label_showinfo_text).grid(row=2,column=0,columnspan=8,sticky='w')
#label_showinfo_text.set("")
#工作记录表格
yscroll = Scrollbar(frame2, orient=VERTICAL, width=10)
...
label_monthwork_info_text= tk.StringVar()
label_monthwork = tk.Label(frame2, textvariable=label_monthwork_text).grid(row=2)
label_monthwork_info=tk.Label(frame2, textvariable=label_monthwork_info_text).grid(row=1)
label_monthwork_info_text.set("*本表格仅显示前40条记录")
#--------------稿件基础信息区--------------

label_book_name = tk.Label(frame4, text="稿件名称：").grid(row=1, column=0,padx=10)
...
label_book_info = tk.Label(frame4, text="类型：").grid(row=1, column=7)
book_info_var = tk.StringVar()
box_book_info=ttk.Combobox(frame4, width=10,textvariable=book_info_var)
#entry_book_info = tk.Entry(frame4, width=15,textvariable=book_info_var)
#entry_book_info.grid(row=1, column=8)
box_book_info.grid(row=1, column=8)
box_book_info['value']=('原稿','校样','其他')
box_type.configure(state="readonly")
#稿件信息表格
yscroll_book = Scrollbar(frame5, orient=VERTICAL, width=10)
...
def refresh():
    for item in tree.get_children():
        tree.delete(item)
    select_all_cmd = "SELECT * FROM scores order by id desc limit 40"
    ...
    for i in range(0,len(result_list)):
        ...
        if (data[len(data) - 1] == 0) & (data[len(data) - 2] == 0):
            ...
            if (data[len(data) - 1] == 1) & (data[len(data) - 2] == 1):
                tree.insert('', 'end', values=data, tags='month&week')
            else:
                if data[len(data) - 1] == 1:
                    tree.insert('', 'end', values=data, tags='month')
                if data[len(data) - 2] == 1:
                    tree.insert('', 'end', values=data, tags='week')
        data.clear()
...
def additem():
    if not existBook(str(entry_name.get())):
    ...
    elif (entry_pagenum.get().isdigit() or isdecimal(entry_pagenum.get()) or entry_pagenum.get() == '') & isxishu(
            entry_xishu.get()):
        #print("添加了新项")
        t = time.localtime()
        ...
        if box_type.get()=="原稿寄回":
            cmd_getwordnum="select 总字数 from book_scores where 稿件名称=(?)"
            cur.execute(cmd_getwordnum,(entry_name.get(),))
            conn.commit()
            wordnum_tmp=cur.fetchone()
            print("wordnum_tmp:",wordnum_tmp)
            updateBookState(entry_name.get())
            current_wordnum=str(wordnum_tmp[0])
        else:current_wordnum=getCurrentWorkWordnum(pagenum,entry_name.get()) #根据页数与总字数计算本次加工字数
        if box_type.get()=="校样寄回":
            updateBookState(entry_name.get())
            current_wordnum=str(0)
        cmd_add_item = "INSERT INTO scores VALUES(NULL,'" + today + "','" + weekday + "','" + entry_name.get() + "','" + box_type.get() + "'," + str(
            pagenum) + "," + current_wordnum + "," + entry_xishu.get() + ",0,0); "
        cur.execute(cmd_add_item)
        conn.commit()
        name_var.set('')
        type_var.set('')
        wordnum_var.set('')
...
def deleteRecent():
    ...
    conn.commit()
    res_t=cur.fetchall()
    if res_t[0][1]=="原稿寄回"or res_t[0][1]=="校样寄回":
...
def exAndSave_WeekRpt(modelpath, savepath, extext):
    ...
    for p in doc.paragraphs:
        if '{{text1}}' in p.text:
            p.text = p.text.replace('{{text1}}', extext[0]+extext[1])
        new_file.write(p.text + '\r')
    new_file.close()
    return


def booknameOrNum_afterProcess(param):
    result=param
    #underline = u'\033[4m'
    #end = u'\033[0m'
    if "(原稿)"in param or "(校样)"in param or "(其他)"in param:
        result=param[0:len(param)-len("(原稿)")]
    #result=underline+result+end
    return result
...
def gentext(type,data):
    ...
    if type=="月报":
        for i in range(0, len(data)):
            if data[i][1] == '第一遍加工':
                text_res_1= text_res_1 + "稿件名称 《" + booknameOrNum_afterProcess(data[i][0]) + "》 字数 " + booknameOrNum_afterProcess(str(data[i][3])) + "万字（第一遍）\n"
                print((text_res_1))
            elif data[i][1] == '原稿寄回':
                text_res_1= text_res_1 + "稿件名称 《" + booknameOrNum_afterProcess(data[i][0]) + "》 字数 " + str(data[i][3]) + "万字（第二遍，已寄回）\n"
            elif data[i][1] == '读校样':
                text_res_2= text_res_2 + "稿件名称 《" + booknameOrNum_afterProcess(data[i][0]) + "》 字数 " + str(data[i][3]) + "万字\n"
    elif type=="周报":
        for i in range(0, len(data)):
            if data[i][1] == '第一遍加工':
                text_res_1= text_res_1 + "加工稿件 《" + booknameOrNum_afterProcess(data[i][0])+ "》 第一遍" + str(data[i][2]) + "页。"
            elif data[i][1] == '第二遍加工':
                text_res_1= text_res_1 + "加工稿件 《" + booknameOrNum_afterProcess(data[i][0]) + "》 第二遍 " + str(data[i][2]) + "页。"
            elif data[i][1] == '读校样':
                text_res_2= text_res_2 + "读校样 《" + booknameOrNum_afterProcess(data[i][0]) + "》 " + str(data[i][2]) + "页。"
...
def creatsettingWindow():
    ...
    label_MonthSavePath = tk.Label(settingWindow, textvariable=MonthSavePath).grid(row=3, column=1)
...
but_weekRpt = tk.Button(frame3, text="生成周报", command=lambda: genweekRpt()).grid(row=0, column=0, padx=5, pady=3)
but_monthRpt = tk.Button(frame3, text="生成月报", command=lambda: genmonthRpt()).grid(row=0, column=1, padx=5, pady=3)
...
but_delereall = tk.Button(frame3, text="删除全部", command=lambda: [deleteAllorNot(), updata_monthwork()]).grid(
...
def addbook():
    if (entry_book_pagenum.get().isdigit()  or entry_book_pagenum.get() == '' ) & (entry_book_wordnum.get().isdigit() or isdecimal(entry_book_wordnum.get())):
        print("添加了稿件")
        if entry_book_pagenum.get() == '':
            book_pagenum = 0
        else:
            book_pagenum = entry_book_pagenum.get()
        cmd_add_book = "INSERT INTO book_scores VALUES(NULL,'" + entry_book_name.get() +'('+box_book_info.get()+')'+ "',"+ str(book_pagenum) + "," + entry_book_wordnum.get() +",'"+box_book_info.get()+"',0); "
